...
export const actualizarAsistencia = async (req, res) => {
  try {
    ...
    // Notificar cambio a través de SSE para actualización en tiempo real
    try {
      const asistencia = await AsistenciaService.obtenerAsistencia(req.params.id);
      if (asistencia) {
        const [fechaRows] = await pool.query(
          'SELECT fecha FROM programacion_fechas WHERE id = ?',
          [asistencia.programacion_fecha_id]
        );
        const fecha = fechaRows[0]?.fecha;
        if (fecha) {
          notificationService.notifyAsistenciaChange(asistencia.subalmacen_id, fecha, 'update');
        }
      }
    } catch (notifError) {
      console.error('[actualizarAsistencia] Error notificando cambio SSE:', notifError);
    }
...
export const eliminarAsistencia = async (req, res) => {
  try {
    // Obtener datos antes de eliminar para notificar SSE
    const asistencia = await AsistenciaService.obtenerAsistencia(req.params.id);
    ...
    try {
      if (asistencia) {
        ...
        const fecha = fechaRows[0]?.fecha;
        if (fecha) {
          notificationService.notifyAsistenciaChange(asistencia.subalmacen_id, fecha, 'delete');
        }
      }
    } catch (notifError) {
      console.error('[eliminarAsistencia] Error notificando cambio SSE:', notifError);
    }
