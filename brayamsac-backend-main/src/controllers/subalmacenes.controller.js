...
export const crearSubalmacen = async (req, res) => {
  ...
  try {
    ...
    notificationService.notifyDashboardUpdate('subalmacen', 'create');
...
export const actualizarSubalmacen = async (req, res) => {
  ...
  try {
    ...
    if (affectedRows === 0)
      return res.status(404).json({ error: 'Subalmacén no encontrado' });
    notificationService.notifyDashboardUpdate('subalmacen', 'update');
...
export const eliminarSubalmacen = async (req, res) => {
  try {
    ...
    if (affectedRows === 0)
      return res.status(404).json({ error: 'Subalmacén no encontrado' });
    notificationService.notifyDashboardUpdate('subalmacen', 'delete');
    res.json({ mensaje: 'Subalmacén y todos sus datos asociados eliminados correctamente' });
  } catch (error) {
    console.error('❌ Error al eliminar subalmacén:', error.message);
    
    // Manejar errores de clave foránea específicamente
    if (error.code === 'ER_ROW_IS_REFERENCED_2' || error.message.includes('foreign key')) {
      return res.status(400).json({ 
        error: 'No se puede eliminar el subalmacén porque tiene datos asociados que no pudieron ser removidos.' 
      });
    }
    
    res.status(500).json({ error: 'Error al eliminar subalmacén: ' + error.message });
  }
};
